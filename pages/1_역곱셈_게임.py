...
if "game_score" not in st.session_state:
    st.session_state.game_score = 0  # 이번 판 맞춘 문제 수
    st.session_state.inputs = []
    st.session_state.status = "playing"  # playing, hint
...
def press_key(val):
    if st.session_state.is_answered or st.session_state.answer_processed:
        return
...
# 2. 동물 데이터 정의 (메인 app.py와 Key 이름 명확히 일치시킴)
animals_data = {
...
if st.session_state.gacha_step == "shaking":
    ...
    st.markdown(f"<div class='animal-name'>[{tier.split()[0]}] {animal.split()[-1]}</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    
    cols_action = st.columns(2)
    with cols_action[0]:
        if st.button("도감 확인하러 가기", use_container_width=True):
            st.session_state.gacha_step = "idle"
            st.switch_page("streamlit_app.py")  # 도감으로 즉시 사후 강제 이동
    with cols_action[1]:
...
if st.session_state.gacha_step == "idle":
        ...
        for row in key_matrix:
            ...
            for i, num in enumerate(row):
                pad_cols[i].button(
                    ...
                    disabled=st.session_state.is_answered or st.session_state.answer_processed,
                    on_click=press_key,
                    args=(num,),
                )
    ...
    # 정답 자동 판별 알고리즘
    if len(st.session_state.inputs) == 2 and st.session_state.answer_processed:
        time.sleep(0.4)  # 정답 확인 가독성 버퍼 타임
        ...
        if user_ans1 * user_ans2 == st.session_state.target_product:
            ...
            st.session_state.gold += reward_gold  # 전역 지갑 실시간 누적
            time.sleep(1.5)
            next_question()
            st.rerun()
        else:
