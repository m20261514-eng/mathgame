...
st.set_page_config(page_title="신비의 알 곱셈 퀘스트", page_icon="🥚", layout="centered")
...
with cols_nav[0]: 
    st.markdown("<div style='padding-top: 5px;'><h2 class='game-title'>⚔️ 곱셈 게임</h2></div>", unsafe_allow_html=True)
with cols_nav[1]:
    ...
    for row in key_matrix:
        ...
        for i, num in enumerate(row):
            if pad_cols[i].button(str(num), key=f"pad_{num}", use_container_width=True, disabled=is_keyboard_locked):
                if len(st.session_state.inputs) < 2: # 구구단의 최대 결과값은 2자리수(81)이므로 제한
    ...
    if last_row_cols[2].button("확인", key="pad_enter", use_container_width=True, disabled=is_keyboard_locked):
        if st.session_state.inputs:
            ...
            if user_val == st.session_state.target_answer:
                ...
                # 오답 처리
                st.session_state.inputs = [] # 입력칸 초기화
                st.toast("앗! 다시 계산해볼까요? ❌") # 귀여운 경고 알림
                st.rerun()
