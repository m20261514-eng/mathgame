...
# 2. 동물 데이터 정의 (메인 app.py와 Key 이름 완벽 일치시킴)
animals_data = {
...
if st.session_state.gacha_step == "shaking":
    ...
    with cols_action[0]:
        if st.button("도감 확인하러 가기", use_container_width=True):
            st.session_state.gacha_step = "idle"
            st.switch_page("streamlit_app.py")  # 뽑은 직후 도감 로비로 화면 전송
    with cols_action[1]:
...
if st.session_state.gacha_step == "idle":
    ...
    if len(st.session_state.inputs) == 1 and st.session_state.is_answered and st.session_state.status == "playing":
        ...
        if user_answer == st.session_state.correct_answer:
            ...
            st.session_state.gold += get_gold  # 전역 공용 계좌 반영
